...
{
	let "TASKNO += 1"
	echo -e "\n\n overall progress : $TASKNO / 30" > ${TASKPROGIDFILE}
}
...
function task3()
{
	...
	local di=0
	#local binchoices=(touch touch nontouch touch nontouch)
	local binchoices1=(touch nontouch nontouch touch touch)
	local binchoices2=(touch touch touch nontouch nontouch)
	local binchoices3=(nontouch nontouch touch nontouch touch)
	local binchoices4=(nontouch nontouch touch touch nontouch)
	local binchoices5=(nontouch nontouch nontouch touch touch)
	...
	do
		...
		echo -e "\n##### with Normal case #####" >> $LOG
		echo "##### under seeding resolution of ${n}x${n}x${n} #####" >> $LOG
		echo "#####    when the tumor $CURCHOICE fibers    #### ">> $LOG
		...
		-i \
		${NORMALCASEDATADIR}/region_for_task3/s${n}/${CURFB}/${CURCHOICE}/tumorbox_0_region_s${n}.data \
		-u \
...
function task5()
{
	...
	local binchoices1=(no no yes no no)
	local binchoices2=(yes no no no yes)
	local binchoices3=(no yes yes yes no)
	local binchoices4=(yes no no yes yes)
	local binchoices5=(yes yes yes no no)
	let "pidx = pi + 1"
...
function task6()
{
	...
	local trichoices1=(rightcut leftcut rightcut nocut nocut)
	local trichoices2=(nocut leftcut rightcut leftcut leftcut)
	local trichoices3=(leftcut rightcut leftcut nocut rightcut)
	local trichoices4=(rightcut leftcut nocut nocut rightcut)
	local trichoices5=(nocut leftcut rightcut rightcut leftcut)
	let "pidx = pi + 1"
