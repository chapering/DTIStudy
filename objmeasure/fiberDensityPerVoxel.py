...
VOXELSIZE=(1.0,1.0,.997059)
VOXELDIMENSION=sqrt( VOXELSIZE[0]**2 + VOXELSIZE[1]**2 + VOXELSIZE[2]**2 )
print "voxel volume: %f" % (VOXELDIMENSION)
...
def compVertices(v1, v2): # compare two vertices
	...
	for i in range(0,3):
		if not compf( v1[i], v2[i] ):
			return False
	'''
	dist = sqrt ( (v1[0]-v2[0])**2 + (v1[1]-v2[1])**2 + (v1[2]-v2[2])**2 )
	if dist < VOXELDIMENSION:
		return True
...
def calc():
	...
	for voxel in g_allvoxels:
		...
		voxelStat[voxel] = findVoxelInTube(voxel, alllines)
	...
	print >> sys.stdout, "Tract Volume = %f " % \
			(VOXELSIZE[0]*VOXELSIZE[1]*VOXELSIZE[2]*totalVoxel* (g_s**3))
