...
def getavg():
	global g_allfbnum
	for _cls in ("wholebrain", "normal_whole", "normal_allfb", "normal_partial"):
		cls = _cls
		if _cls == "wholebrain":
			cls = "normal_whole"
		for res in range(1,6):
			...
			for _tid in range(1,7):
				...
				for fb in ("cst", "cg", "cc", "ifo", "ilf"):
					if _cls=="wholebrain" and fb!="cc":
						continue
					if _cls=="normal_whole" and fb=="cc":
						continue
					for pos in ("", "pos1"):
			...
			if _cls not in g_allfbnum.keys():
				g_allfbnum[_cls] = dict()

			if res not in g_allfbnum[_cls].keys():
				g_allfbnum[_cls][res] = [.0,.0,.0]

			g_allfbnum[_cls][res][0] = fbtotal*1.0/itemnum
			...
			for _tid in range(1,7):
				...
				for fb in ("cst", "cg", "cc", "ifo", "ilf"):
					...
					for pos in ("", "pos1"):
						...
						fbtotal += (fbnum - g_allfbnum[_cls][res][0])**2
						itemnum += 1

			g_allfbnum[_cls][res][1] = math.sqrt( fbtotal*1.0/itemnum )
			g_allfbnum[_cls][res][2] = math.sqrt( fbtotal*1.0/itemnum ) / math.sqrt( itemnum*1.0 )

def dump():
	global g_allfbnum
	#print >> sys.stdout, "data\t\ts1\t\ts2\t\ts3\t\ts4\t\ts5"
	print >> sys.stdout, "data category\t\tres\t\tavg\t\tstdev\t\tstderr"
	print >> sys.stdout, "==========================================" \
							"=========================================="
