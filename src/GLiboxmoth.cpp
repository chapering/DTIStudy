...
void CGLIBoxApp::display ( void )
{
	...
	/* if we just want to go over all tasks to capture snapshots, deannotate 
	 * the following block and rebuild the package
#ifdef CORONA
	glDumpImage((m_width-m_height)/2, 0, m_height, m_height);
#else
	glDumpImagePPM((m_width-m_height)/2, 0, m_height, m_height,false);
#endif
	cleanup();
	exit(0);
	 */
