...
def dump():
	...
	print >> sys.stdout, "Pid Tid Torder Res Dcls Did Time Correct Ans Key ec1 ec2 ec3 ec4 ec5"
		...
		for tid in g_allparticipantinfo[pidx].keys():
			for blockId in range(0, blockNum):
				...
				for res in g_allparticipantinfo[pidx][tid].keys():
					...
					elif tid == 4 and len(ta)>=4 and
					len(g_alllesioninfo)>blockId: # lesion size and number of broken fibers
					...
					print >> sys.stdout, "%d %d %d %s %d %d %d %d %s %s %s" % \
						(pidx+1, tid, tnum, res[0], dcls, did, ta[0], ta[1], ta[2], key, dprop.rstrip("\r\n"))
					tnum+=1
					bfid+=1
