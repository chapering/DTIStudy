	...
	while curline:
		...
		if string.find( curline, FORANSWER ) != -1:
			...
			if resolution not in participantInfo[ taskid ].keys():
				participantInfo[ taskid ][ resolution ] = [0]
			#participantInfo[ taskid ][ resolution ].append( (singletasktime, answer ) )
			# drop the first block, keep the last one for each task only
			participantInfo[ taskid ][ resolution ][0]= (singletasktime, answer ) 
...
def plot_completiontime():
	...
	plt.legend( (alltaskrect[0], alltaskrect[1], alltaskrect[2], alltaskrect[3],
		alltaskrect[4], alltaskrect[5], alltaskrect[6]), ("Task 1:FA comparison",
			"Task 2: Fiber tracing", "Task 3: bundle recognition", 
			"Task 4: lesion - partial brain", "Task 5: bundle membership",
			"Task 6: symm bundle size difference", "Task 7: lesion - single bundle") )
