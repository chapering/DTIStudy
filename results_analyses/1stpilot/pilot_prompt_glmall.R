...
cat( sprintf("### AVOVA for all tasks\n") )
	cat( sprintf("#\t Time ~ Res \n") )
	boxplot( data1$Time~data1$Res, main=sprintf("AVOVA of all Tasks - Time~Res") )
	TX <- aov( data1$Time~data1$Res )
	print (TX)
	print( summary(TX) )

	cat( sprintf("#\t Correct ~ Res \n") )
	boxplot( data1$Correct~data1$Res, main=sprintf("AVOVA of all Tasks - Correct~Res") )
	CX <- aov( data1$Correct~data1$Res )
	print (CX)
	print( summary(CX) )
...
cat( sprintf("\n### linear regression model for all tasks\n") )
	cat( sprintf("#\t Time ~ Res \n") )
	tfit <- lm(data1$Time~data1$Res)
	print( summary( tfit ) )
	par(mfrow=c(2,2)); plot( tfit, main=sprintf("regression model of all Tasks - Time~Res") )

	cat( sprintf("#\t Correct ~ Res \n") )
	cfit <- lm(data1$Correct~data1$Res)
	print( summary( cfit ) )
	par(mfrow=c(2,2)); plot( cfit, main=sprintf("regression model of all Tasks - Correct~Res") )
...
########################################################################
cat( sprintf("\n### generalized linear model (GLM) for all tasks --- using Gaussian model\n") )
	cat( sprintf("#\t Time ~ Res \n") )
	tgfit <- glm(data1$Time~data1$Res)
	print( summary( tgfit ) )
	print( anova( tgfit ) )
	par(mfrow=c(2,2)); plot( tgfit, main=sprintf("GLM of all Tasks - Time~Res") )

	cat( sprintf("#\t Correct ~ Res \n") )
	cgfit <- glm(data1$Correct~data1$Res)
	print( summary( cgfit ) )
	print( anova( cgfit ) )
	par(mfrow=c(2,2)); plot( cgfit, main=sprintf("GLM of all Tasks - Correct~Res") )


	cat( sprintf("#\t Time ~ Res*Tid : gaussian model \n") )
	tgfit <- glm(data1$Time~data1$Res*data1$Tid, family="gaussian")
	print( summary( tgfit ) )
	print( anova( tgfit ) )
	par(mfrow=c(2,2)); plot( tgfit, main=sprintf("GLM of all Tasks - Time~Res*Tid") )

	cat( sprintf("#\t Correct ~ Res*Tid : gaussian model \n") )
	cgfit <- glm(data1$Correct~data1$Res*data1$Tid, family="gaussian")
	print( summary( cgfit ) )
	print( anova( cgfit ) )
	par(mfrow=c(2,2)); plot( cgfit, main=sprintf("GLM of all Tasks - Correct~Res*Tid") )

cat( sprintf("\n### GLM for each task -- using Gaussian model\n") )
for (i in 1:7) {
	cat( sprintf("##========= GLM of Task %d =========\n", i) )
	tdata <- data1 [ data1$Tid == i, ]
	cat( sprintf("#\t Time ~ Res \n") )
	tfit <- glm(tdata$Time~tdata$Res)
	print( summary( tfit ) )
	par(mfrow=c(2,2)); plot( tfit, main=sprintf("GLM of Task %d - Time~Res", i) )

	cat( sprintf("#\t Correct ~ Res \n") )
	cfit <- glm(tdata$Correct~tdata$Res)
	print( summary( cfit ) )
	par(mfrow=c(2,2)); plot( cfit, main=sprintf("GLM of Task %d - Correct~Res", i) )
}


cat( sprintf("\n### GLM for all tasks -- using Poisson model\n") )
	cat( sprintf("#\t Time ~ Res*Tid : poisson model \n") )
	tgfit <- glm(data1$Time~data1$Res*data1$Tid, family="poisson")
	print( summary( tgfit ) )
	print( anova( tgfit ) )
	par(mfrow=c(2,2)); plot( tgfit, main=sprintf("GLM of all Tasks - Time~Res*Tid") )

	cat( sprintf("#\t Correct ~ Res*Tid : poisson model \n") )
	cgfit <- glm(data1$Correct~data1$Res*data1$Tid, family="poisson")
	print( summary( cgfit ) )
	print( anova( cgfit ) )
	par(mfrow=c(2,2)); plot( cgfit, main=sprintf("GLM of all Tasks - Correct~Res*Tid") )


cat( sprintf("\n### GLM for each task -- using Poisson model\n") )
for (i in 1:7) {
	cat( sprintf("##========= GLM of Task %d =========\n", i) )
	tdata <- data1 [ data1$Tid == i, ]
	cat( sprintf("#\t Time ~ Res \n") )
	tfit <- glm(tdata$Time~tdata$Res, family="poisson")
	print( summary( tfit ) )
	par(mfrow=c(2,2)); plot( tfit, main=sprintf("GLM of Task %d - Time~Res", i) )

	cat( sprintf("#\t Correct ~ Res \n") )
	cfit <- glm(tdata$Correct~tdata$Res, family="poisson")
	print( summary( cfit ) )
	par(mfrow=c(2,2)); plot( cfit, main=sprintf("GLM of Task %d - Correct~Res", i) )
}
