...
def ParseCommandLine():
	...
	for i in range(1,10):
		if os.path.isfile("./bundleDiffs_block%d" % i):
			...
			curline="#"
			while curline:
				curline = hfbd.readline()
				curline = curline.lstrip().rstrip(' \r\n')
				if len(curline) < 1:
					continue
				tmplist.append(curline)
			hfbd.close()
			g_allBundlesizeDiffs.append(tmplist)
...
def dump():
	...
	print >> sys.stdout, "Pid Tid Torder Res Did Time answer Correct Dprop"
			...
			for blockId in range(0, blockNum):
				...
				for res in g_allparticipantinfo[pidx][tid].keys():
					...
					if tid == 1 and len(g_allFADiffs)>blockId: # FA difference
						...
						realanswer = "%s %s" % (ta[2], ta[1])
						dprop = "%d %s" % (key, dprop)
					elif (tid == 4 or tid==7 ) and len(ta)>=3:
						dprop= ta[2] 

					print >> sys.stdout, "%d %d %d %s %s %d %s %s" % \
						(pidx+1, tid, tnum, res[0], did, ta[0], realanswer, dprop)
					tnum+=1
					bfid+=1
