...
# only consider the normal_whole dataset
data1 <- alldata[ ! ((Torder%%30 >= 11) & (Torder%%30 <=20)), ]
...
correctdata1 <- alldata[! (alldata$Correct==0), ]
...
cat( sprintf("### AVOVA for all tasks\n") )
	...
	X <- aov( data1$Time~as.factor(data1$Res))
	print (X)
	print( summary(X) )
	cat( sprintf("#\t Tukey HSD of Time ~ Res\n") )
	kX <- TukeyHSD( X )
	print (kX)
	plot ( kX )
	...
	Y <- aov( data1$Correct~as.factor(data1$Res) )
	print (Y)
	print( summary(Y) )
	cat( sprintf("#\t Tukey HSD of Correct ~ Res \n") )
	kY <- TukeyHSD( Y )
	print (kY)
	plot ( kY )
...
for (i in 1:6) {
	...
	TX <- aov( tdata$Time~as.factor(tdata$Res) )
	print (TX)
	print( summary(TX) )
	cat( sprintf("#\t Tukey HSD of Time ~ Res\n") )
	tkX <- TukeyHSD( TX )
	...
	CX <- aov( tdata$Correct~as.factor(tdata$Res) )
	print (CX)
	print( summary(CX) )
	cat( sprintf("#\t Tukey HSD of Correct ~ Res \n") )
	ckX <- TukeyHSD( CX )
...
for (i in 1:6) {
	...
	tdata <- data1 [ data1$Tid == i, ]
	cat( sprintf("#\t Time ~ Res \n") )
	tfit <- lm(tdata$Time~tdata$Res+tdata$Did)
	print( summary( tfit ) )
	par(mfrow=c(2,2)); plot( tfit, main=sprintf("regression model of Task %d - Time~Res", i) )

	cat( sprintf("#\t Correct ~ Res \n") )
	cfit <- lm(tdata$Correct~tdata$Res+tdata$Did)
	print( summary( cfit ) )
