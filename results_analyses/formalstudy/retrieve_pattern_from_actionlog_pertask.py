...
separate files, but all in the VTK legacy format. Each vtk file will only involves a single task

Copyright Chapstudio 2010-2011
Haipeng Cai @Aug. 17th, 2011
'''
		...
		for tid in g_allparticipantinfo[pidx].keys():
			...
			for blockId in range(0, blockNum):
				...
				if dcls not in vtkpolydatainfo[tid]:
					vtkpolydatainfo[tid][dcls] = odict()

				dblk = blockId % 2 + 1
				if dblk not in vtkpolydatainfo[tid][dcls].keys():
					vtkpolydatainfo[tid][dcls][dblk] = odict()
				...
				for res in g_allparticipantinfo[pidx][tid].keys():
					...
					if did not in vtkpolydatainfo[tid][dcls][dblk].keys():
						hfn = file ("%s/t%d_cls%d_b%d_d%d_%s.vtk" % (outputdir, tid, dcls, dblk, did, g_fns[pidx]),'w')
						vtkpolydatainfo[tid][dcls][dblk][did] = [hfn,0,0]

					for subrot in ta[2]:
						vtkpolydatainfo[tid][dcls][dblk][did][1] += 1
						vtkpolydatainfo[tid][dcls][dblk][did][2] += len (subrot[1])
			...
			for dcls in (1,2,3):
				for dblk in (1,2):
					for did in (1,2,3,4,5):
						hfn = vtkpolydatainfo[tid][dcls][dblk][did][0]
						ptnum = vtkpolydatainfo[tid][dcls][dblk][did][2]
						''' VTK legacy file header '''
						print >> hfn, "# vtk DataFile Version 1.0"
						print >> hfn, "User action in DTI density experiment : Task-%d, Class-%s, Block-%d Data-%s" % \
									(tid, DataClasses[dcls-1], dblk, DataCategories[did-1])
						print >> hfn, "ASCII"
						print >> hfn
		...
		for tid in g_allparticipantinfo[pidx].keys():
			for dcls in (1,2,3):
				for dblk in (1,2):
					for did in (1,2,3,4,5):
						hfn = vtkpolydatainfo[tid][dcls][dblk][did][0]
						linenum = vtkpolydatainfo[tid][dcls][dblk][did][1]
						ptnum = vtkpolydatainfo[tid][dcls][dblk][did][2]
						print >> hfn
			...
			for dcls in (1,2,3):
				for dblk in (1,2):
					for did in (1,2,3,4,5):
						...
						hfn.close()
		print >> sys.stderr, "processing for participant %d finished." % (pidx+1)
