...
'''data structure:
	[ {taskid:{res:[(singletasktime, did, [(rottimespan, [(time,rx,ry,rz,angle,dist),...]),...],...),...],...}, ...}, ... ]
'''
...
def loadsingle(srcfn):
	...
	while curline:
		...
		if string.find( curline, FORANGLE ) != -1:
			...
			if len(rotinfo) < 4:
				raise ValueError, "parsing %s failed at line No. %d - %s." % (srcfn,lnCnt,curline)
			dist = sqrt( (rotinfo[1]-prerotinfo[1])**2 + (rotinfo[2]-prerotinfo[2])**2 + (rotinfo[3]-prerotinfo[3])**2 )
			if rottime-rottimeorigin > 1e-3 :
				subrotlist.append( (rottime-rottimeorigin, prerotinfo[0], prerotinfo[1], prerotinfo[2], prerotinfo[3], predist) )
			...
			for blockId in range(0, blockNum):
				...
				for res in g_allparticipantinfo[pidx][tid].keys():
					...
					if did not in vtkpolydatainfo[tid][dcls][1].keys():
						hfn = file ("%s/p%d_t%d_cls%d_d%d.vtk" % (outputdir, pidx+1, tid, dcls, did),'w')
