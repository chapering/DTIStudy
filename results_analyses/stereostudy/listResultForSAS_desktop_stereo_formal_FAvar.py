...
COMPAREDRES="compared with seeding resolution of"
...
def loadsingle(srcfn):
	...
	def pickCompRes( curline ) :
		compResRet = (-1,-1,-1)
		curline = curline.lstrip().rstrip(' \r\n')
		if len (curline) > 10 and string.find( curline, COMPAREDRES ) != -1:
			resinfo = split ( curline )
			if len(resinfo) >= 5:
				resinfo = split ( resinfo[5].lstrip().rstrip(" ,"), 'x', 3 )
				compResRet = (resinfo[0], resinfo[1], resinfo[2])
		return compResRet
	...
	while curline:
		...
		if string.find( curline, COMPAREDRES ) != -1:
			compres = pickCompRes( curline )
			if compres == (-1,-1,-1):
				raise ValueError, "parsing %s failed at line No. %d - %s." % (srcfn,lnCnt,curline)
			curline = sfh.readline()
			continue
		...
		if string.find( curline, FORANSWER ) != -1:
			...
			if clkdist >= 0.0:
				...
				participantInfo[ taskid ][ resolution ].append( (singletasktime, hesitime, curfbindices[fbidx], answer, tanswer, compres) )
				compres = (-1,-1,-1)
			else:
...
def dump():
	...
	print >> sys.stdout, "Pid Tid Torder Res Dcls Did Bname Time htime Correct Ans Key ec1 ec2 ec3 ec4 ec5 ec6 ec7 stereo"
			...
			for blockId in range(0, blockNum):
				...
				for res in g_allparticipantinfo[pidx][tid].keys():
					...
					if tid == 1 and len(g_allFADiffs)>blockId: # FA difference
						...
						dprop="%s 0 0 0 0 0 0"%( g_allmfiber_t2[blockId][ (did-1)*5+int(res[0])-1 ].rstrip("\r\n") )
						key=TASK2KEYS[did-1][int(res[0])-1]

					elif tid == 3 and len(g_allmfiber_t3)>blockId: # number of marked fibers
						dprop="%s 0 0 0 0 0 0"%( g_allmfiber_t3[blockId][ (did-1)*5+int(res[0])-1 ].rstrip("\r\n") )
						key=did

					elif tid == 4 and len(ta)>=6 and len(g_alllesioninfo)>blockId: # lesion size and number of broken fibers
						dprop="%s %s 0 0 0 0"%( ta[5], g_alllesioninfo[blockId][(did-1)*5+int(res[0])-1].rstrip("\r\n"))
						key=1

					elif tid == 5:
						dprop="%s 0 0 0 0 0" % ( g_allSNinfo[blockId][(did-1)*5+int(res[0])-1].rstrip("\r\n") )
						...
						dprop = g_allBundlesizeDiffs[blockId][(did-1)*3*5+(int(res[0])-1)*3 + \
									CUTSCHEME[ blockId*5 + pidx%5 ][ t6did ]]
						dprop_words = split(dprop)
						prop = float(dprop_words[0])
						'''
						dprop = "%s %s" % (res[0], ta[5][0])
						if res[0] == ta[5][0]:
							key = 1
						else:
							key = 2
						dprop="%s 0 0 0 0 0" % ( dprop.rstrip("\r\n") )

					print >> sys.stdout, "%d %d %d %s %d %d %s %.2f %.2f %d %s %s %s %s" % \
